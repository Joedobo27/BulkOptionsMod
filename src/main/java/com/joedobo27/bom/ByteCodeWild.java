...
public class ByteCodeWild {
    ...
    private int findCode(byte[] search) throws NotFoundException {
        ...
        for(int backtrack = 0; i < code.length && j < search.length; ++i) {
            if (code[i] == search[j] || isWildBranching(search, j)) {
                if (j == 0) {
                    backtrack = i;
                }
    ...
    private boolean isWildBranching(byte[] code, int ordinal) {
        if (isBranchingOpCode(code[ordinal]) && code[ordinal + 1] == 0 && code[ordinal + 2] == 0)
            return true;
        else if(ordinal > 0 && isBranchingOpCode(code[ordinal - 1]) && code[ordinal] == 0 && code[ordinal + 1] == 0)
            return true;
        else return ordinal > 1 && isBranchingOpCode(code[ordinal - 2]) && code[ordinal - 1] == 0 && code[ordinal] == 0;
    }
